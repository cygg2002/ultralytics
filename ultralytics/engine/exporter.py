...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if is_tf_format:  # TensorFlow formats
            ...
            if pb:  # pb format is deprecated
                ...
                f[6] = ""  # Skip pb export
            if tflite:  # TFLite export
                f[7] = self.export_tflite()
            if edgetpu:  # EdgeTPU format is deprecated
                LOGGER.warning(
                    "TensorFlow Edge TPU export is deprecated due to incompatibility with ai-edge-torch. "
                    "Use `format=tflite` with INT8 quantization instead for edge deployment."
                )
                f[8] = ""  # Skip edgetpu export
    ...
    def export_edgetpu(self, tflite_model="", prefix=colorstr("Edge TPU:")):
        """Export YOLO model to Edge TPU format (DEPRECATED).

        Notes:
            Edge TPU export is deprecated due to incompatibility with ai-edge-torch. The EdgeTPU compiler
            does not support many modern TFLite ops generated by ai-edge-torch. For edge deployment,
            use TFLite format with INT8 quantization instead:
            `yolo export model=yolo11n.pt format=tflite int8=True data=coco8.yaml`
        """
        raise NotImplementedError(
            f"{prefix} Edge TPU export is deprecated due to incompatibility with ai-edge-torch. "
            "The EdgeTPU compiler does not support modern TFLite ops. "
            "Use TFLite format with INT8 quantization instead: "
            "`yolo export model=yolo11n.pt format=tflite int8=True data=coco8.yaml`"
        )
